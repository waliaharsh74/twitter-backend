...
const extraResolver = {
    User: {
        ...
        recommendedUsers: (parent, _, ctx) => __awaiter(void 0, void 0, void 0, function* () {
            if (!ctx.user)
                return [];
            const cachedValue = yield redis_1.redisClient.get(`recommendeduser:${ctx.user.id}`);
            ...
            yield redis_1.redisClient.set(`recommendeduser:${ctx.user.id}`, JSON.stringify(users));
            return users;
        }),
    },
};
const mutations = {
    followUser: (parent_1, _a, ctx_1) => __awaiter(void 0, [parent_1, _a, ctx_1], void 0, function* (parent, { to }, ctx) {
        ...
        const cachedeleted = yield redis_1.redisClient.del(`recommendeduser:${ctx.user.id}`);
